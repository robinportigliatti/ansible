...
# Author: https://raw.githubusercontent.com/lorello/nagios-checks/master/check-opcache.php
# Edited: ztodorov@neterra.net - remove some output values
# Edited: Jo Goossens jo.goossens@hosted-power.com - added check for interned strings buffer
#
# version: 0.13
...
$interned_strings_buffer_size = $status['interned_strings_usage']['buffer_size'];
$interned_strings_buffer_size_mb = $interned_strings_buffer_size / 1024 / 1024;
...
$start_time_unknown_period = 0;
# debug echo "$start_time + $start_time_unknown_period ? ".time(). "\n";

$result_status = 0;
$result_message = "OK - $basic_info ({$size_mb}MB - {$interned_strings_buffer_size_mb}MB)";
...
# From interned string buffer size
$values['interned_strings_used_memory']                 = $status['interned_strings_usage']['used_memory'];
...
$opcache_hit_rate_warn  = 0.2;
$opcache_hit_rate_crit  = 0.1;
...
$string_usage_perc      = $values['interned_strings_used_memory'] / $interned_strings_buffer_size;
#echo $memory_usage_perc;
$string_memory_usage_perc_warn = 0.8;
$string_memory_usage_perc_crit = 0.9;
if ($string_usage_perc > $string_memory_usage_perc_crit) {
  $result_status    = 2;
  $result_message   = "CRITICAL - $basic_info interned strings buffer memory usage percentage is over critical level ".($string_usage_perc*100).'%';
} elseif ($string_usage_perc > $string_memory_usage_perc_warn) {
  $result_status    = 1;
  $result_message   = "WARNING - $basic_info interned strings buffer memory usage percentage is over warning level ".($string_usage_perc*100).'%';
}
 ...
 *[OK|WARNING|CRITICAL|UNKNOWN] - DataSource1: $type $name $active/max, DataSource2: $type $name $active/$max |
$active;$active_warn;$active_crit;$graph_min;$graph_max
